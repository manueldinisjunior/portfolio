<?php
if(!class_exists("Inbio_Portfolio_Themes_Base")) {
	class Inbio_Portfolio_Themes_Base {
    	public $key = "2C94C4142205EE13";
    	private $product_id = "1";
    	private $product_base = "inbio-personal-portfolio-wordpress-theme";
    	private $server_host = "https://hub.rainbowit.net/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->has_check_update) {
				if(function_exists("add_action")){
					add_action( 'admin_post_inbio-education-themes_fupc', function(){
					...
					if($this->is_theme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if (  plugin_basename( $this->plugin_file ) == $plugin_file ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=inbio-education-themes_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		 ...
		 public function theme_force_update(){
            ...
            echo wp_kses_post('<h1>'.__("Update Checking..","inbio").'</h1>');
