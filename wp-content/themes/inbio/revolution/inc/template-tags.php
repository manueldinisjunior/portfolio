...
if ( ! function_exists( 'art_blog_posted_on' ) ) :
	function art_blog_posted_on() {
		$art_blog_time_string = '<time class="entry-date published updated" datetime="%1$s">%2$s</time>';
		
		if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
			$art_blog_time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time><time class="updated" datetime="%3$s">%4$s</time>';
		}

		$art_blog_time_string = sprintf(
			$art_blog_time_string,
		...
		echo '<span class="posted-on">' . $art_blog_posted_on . '</span>';
	}
endif;

if ( ! function_exists( 'art_blog_posted_by' ) ) :	
...
if ( ! function_exists( 'art_blog_entry_footer' ) ) :	
	function art_blog_entry_footer() {
...
if ( ! function_exists( 'art_blog_post_thumbnail' ) ) :
	function art_blog_post_thumbnail() {
...
if( !function_exists('art_blog_breadcrumbs') ) :
    ...
    function art_blog_breadcrumbs($art_blog_comment = null){
        ...
        echo '<div class="art-blog-breadcrumbs">';
        	art_blog_breadcrumbs_trail();
        echo '</div>';
